...
func loadConfig() {
	...
	if err != nil {
		...
		if os.IsNotExist(err) {
			log.Println("File di configurazione non trovato, ne creo uno nuovo con porta 9012: config.json")
...
func initUsersDB() {
	...
	usersDB, err = sql.Open("sqlite3", "cmd/server/users.db")
	if err != nil {
		log.Fatal(err)
	}
	...
	if count == 0 {
		...
		res, _ := usersDB.Exec("INSERT INTO groups (name, db_path, encryption_key) VALUES ('Default', 'cmd/server/default.db', ?)", defaultKey)
...
func initDB() {
	...
	db, err = sql.Open("sqlite3", "cmd/server/default.db?_foreign_keys=on")
	if err != nil {
		log.Fatal(err)
	}
	migrateDB(db)
...
func main() {
	...
	usersDB.QueryRow("SELECT id FROM groups WHERE db_path = 'cmd/server/default.db'").Scan(&currentWorkspaceID)
	...
	app.Post("/api/admin/groups", authReq, adminReq, func(c *fiber.Ctx) error {
		...
		newDBPath := fmt.Sprintf("cmd/server/%s.db", g.Name)
		// Crea file vuoto
	...
	app.Post("/api/admin/workspace/upload", authReq, adminReq, func(c *fiber.Ctx) error {
		...
		dstPath := fmt.Sprintf("cmd/server/%s.db", safeName)
	...
	app.Post("/api/admin/groups/:id/clone", authReq, adminReq, func(c *fiber.Ctx) error {
		...
		newPath := fmt.Sprintf("cmd/server/%s.db", req.NewName)
	...
	log.Printf("🚀 Server JConMan avviato su %s", serverConfig.ListenAddress)
