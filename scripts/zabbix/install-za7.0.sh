...
# Verifica che sia effettivamente Debian o Ubuntu (evita errori su CentOS/RHEL/Mint)
...
# 3. Generazione dinamica dell'URL del pacchetto
REPO_DEB="zabbix-release_latest_7.0+${OS}${VERSION}_all.deb"
REPO_URL="https://repo.zabbix.com/zabbix/7.0/${OS}/pool/main/z/zabbix-release/${REPO_DEB}"
...
echo "📥 Scaricamento del pacchetto repository per $OS $VERSION..."
wget -qO $REPO_DEB $REPO_URL

# Controllo se il download è andato a buon fine (es. versione OS non supportata da Zabbix)
if [ ! -s $REPO_DEB ]; then
    echo "❌ Errore: Impossibile scaricare il pacchetto. Verifica che Zabbix 7.0 supporti $OS $VERSION."
    rm -f $REPO_DEB
    exit 1
fi
...
echo "✅ Fatto! L'agent Zabbix 2 è installato, configurato e in esecuzione su $NAME."